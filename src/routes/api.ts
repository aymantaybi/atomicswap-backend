...
const web3: Web3 = new Web3(new Web3.providers.WebsocketProvider(WEBSOCKET_PROVIDER!, options));
...
router.get("/reserves", async (req: Request, res: Response) => {
    ...
    var reserves = await Promise.all(pairsAdress.map(pair => getReserves(pair as string, block)));
...
router.get("/pairs", async (req: Request, res: Response) => {
    ...
    if (pathAddresses.length < 2) return res.status(400).json({ error: "Unsupported 'path' query" });

    try {

        var addresses = await Promise.all(pathAddresses.map((address, index) => index == 0 ? null : getPairAddress([address, pathAddresses[index - 1]])));
...
function getPath(tokenIn: string, tokenOut: string) {
    return [tokenIn, tokenOut].map(address => web3.utils.toChecksumAddress(address));
};
